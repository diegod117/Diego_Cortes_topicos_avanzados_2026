...
BEGIN
    FOR pedido IN pedido_cursor LOOP
        ...
        DBMS_OUTPUT.PUT_LINE('Pedido ' || pedido.PedidoID ||
                             ': Nuevo total: ' ||
                             (pedido.Total * (1 + p_porcentaje / 100)));
    END LOOP;
...
/* Prueba (usa el 10% por defecto)
EXEC actualizar_total_pedidos(1);

-- Prueba con porcentaje específico
EXEC actualizar_total_pedidos(2, 15);
*/
...
EXCEPTION
    WHEN NO_DATA_FOUND THEN
        RAISE_APPLICATION_ERROR(-20003, 'Detalle con ID ' ||
                                p_detalle_id || ' no encontrado.');
END;
/
